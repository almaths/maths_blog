...
$$\varrho_n(x_1, \dots , x_n) = C^{\prime \prime}_n \mathrm{e}^{-\sum_{k=1}^n x_i^2} \prod_{1\leq i < j \leq n }|x_i - x_j|^2  .$$
...
where $$\mathrm{d}\nu_\sigma(\lambda) = \frac{1}{\sigma \sqrt{\pi}} \mathrm{e}^{-\left( \frac{\lambda}{\sigma}\right)^2}\, \mathrm{d}\lambda$$ and where $$p_\sigma$$ satifies *integro-differential Painlevé II*,
...
$$\mathcal{F}f(x,\eta) = \frac{1}{\sqrt{2\pi}} \int_{-\infty}^\infty \mathrm{e}^{-i y \eta} f(x,y) \, \mathrm{d}y$$
