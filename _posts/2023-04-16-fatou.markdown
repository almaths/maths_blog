...
[*Update, 20th February 2025:*  After linking this blog post to Google Gemini, I was informed that in fact this result is already known under the title of [Scheffé's lemma](https://en.wikipedia.org/wiki/Scheff%C3%A9%27s_lemma).]
...
$$ \int_X \rho_n(z) \, \mathrm{d}^2 z \longrightarrow \int_X \rho(z) \, \mathrm{d}^2 z $$
...
$$\int_\Omega f_n \, \mathrm{d}\mu \longrightarrow \int_\Omega f \, \mathrm{d}\mu$$

as $$n \to +\infty$$. $$\triangle$$

The difficulty with applying this theorem is to find a suitable "dominating function" $$g \in L^1(\Omega,\mu)$$. In this post I will present a simple lemma that is able to avoid the need to find such a $$g$$. This lemma appears somewhat implicitly in [our paper](https://arxiv.org/abs/2212.00525) from Equations 3.12 to 3.14. I am drawing attention to it since it seems like it would be useful to simplify various proofs; for example, this lemma would have greatly simplified the proofs contained in [my paper](https://arxiv.org/abs/2102.08842) with Mezzadri and Simm on products of truncated orthogonal matrices.
...
$$
\begin{aligned}
&\int_\Omega f_n \, \mathrm{d}\mu =1  & &\forall n \geq 1
\end{aligned}
$$

and 

$$\int_\Omega f \, \mathrm{d}\mu =1$$

Then for any measurable subset $$X \subset \Omega$$ the sequence $$\int_X f_n \, \mathrm{d}\mu $$ is convergent and converges to $$\int_X f \, \mathrm{d}\mu $$. $$\triangle$$

**Proof:** If $$X$$ is measurable then so is $$\Omega \setminus X$$. Thus by the Fatou lemma we have

$$
\begin{aligned}
A&:= \liminf_{n\to \infty}\int_{X} f_n \, \mathrm{d}\mu - \int_{X} f \, \mathrm{d}\mu  \geq 0,\\
B&:= \liminf_{n\to \infty}\int_{\Omega \setminus X} f_n \, \mathrm{d}\mu - \int_{\Omega \setminus X} f \, \mathrm{d}\mu \geq 0.
\end{aligned}
$$

Thus $$A+B \geq 0$$. But $$A+B \leq 0$$ by our assumptions. Hence $$A+B = 0$$ and hence $$A=B=0$$. Thus $$\liminf_{n\to \infty}\int_{X} f_n \, \mathrm{d}\mu = \int_{X} f \, \mathrm{d}\mu$$. 

Now consider a subsequence $$n_k$$ such that $$\int_X  f_{n_k} \, \mathrm{d}\mu \to \limsup_{n \to \infty} \int_X  f_{n} \, \mathrm{d}\mu$$ as $$k \to \infty$$.  We then have the equalities

$$
\begin{aligned}
 \limsup_{n \to \infty} \int_X  f_{n} \, \mathrm{d}\mu = \lim_{k \to \infty}\int_X  f_{n_k} \, \mathrm{d}\mu = \liminf_{k \to \infty}\int_X  f_{n_k} \, \mathrm{d}\mu = \int_X  f \, \mathrm{d}\mu.
\end{aligned}
$$

Thus $$ \limsup_{n \to \infty} \int_X  f_{n} \, \mathrm{d}\mu =  \liminf_{n \to \infty} \int_X  f_{n} \, \mathrm{d}\mu = \int_X  f \, \mathrm{d}\mu$$. $$\square$$