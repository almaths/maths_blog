...
$$\mathbb{E}[| \sigma(X) \cap A |] = \int_A \rho_{N,L}(x) \, \mathrm{d}x $$
...
$$\mathbb{E}[ |\sigma(X) \cap \mathbb{R}|] = \int_\mathbb{R} \rho_{N,L}(x) \, \mathrm{d}x$$
...
$$\rho_{N,L}(x)= \int_{[-1,1]} |x-y| w_L(x) w_L(y) f_{N-2,L}(xy) \, \mathrm{d}y $$
...
$$|f_{\infty,L}(-x)| \leq  \frac{1}{  \left( 1 - x^\frac{1}{m} \right)^{m(L+1)}} \mathrm{e}^{-\frac{Lx^\frac{1}{m}}{2m}}.$$
