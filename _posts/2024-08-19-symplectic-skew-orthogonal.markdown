...
**Proposition:** Let $$C_k = \mathrm{e}^{\frac{2\pi i k}{D} }[0,+\infty)$$ for $$k = 0, \dots, D-1$$. $$C_0$$ has outgoing orientation (away from the origin) and $$C_1, \dots, C_{D-1}$$ have incoming orientation (towards the origin). Then define $$\Gamma_k = C_0 \cup C_k$$ for $$k = 1, \dots, D-1$$, with orientation carried over.
